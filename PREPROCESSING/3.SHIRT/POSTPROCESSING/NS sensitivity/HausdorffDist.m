function [hd,hds,D,cP,cQ] = HausdorffDist(P,Q,lmf,dv)
...
% to any point in Q.
vp = max(min(D,[],2));vps = std(min(D,[],2));
% Obtain the value of the point, q, in Q with the largets minimum distance
% to any point in P.
vq = max(min(D,[],1));vqs = std(min(D,[],1));

hd = max(vp,vq);hds = max(vps,vqs);
...
if nargin==4 && any(strcmpi({'v','vis','visual','visualize','visualization'},dv))
    ...
    % -- plot data --
    if sP(2) == 2
        h(1) = plot(P(:,1),P(:,2),'bx','markersize',3,'linewidth',3);
        h(2) = plot(Q(:,1),Q(:,2),'ro','markersize',3,'linewidth',2.5);
        ...
        h(3) = plot(xyf(:,1),xyf(:,2),'-ks','markersize',8,'linewidth',2);
    ...
    elseif sP(2) == 1   
        ...
        h(1) = plot(P(:,1),ones(1,sP(1)),'bx','markersize',3,'linewidth',3);
        h(2) = plot(Q(:,1),ones(1,sQ(1))+ofst,'ro','markersize',3,'linewidth',2.5);
        % draw all minimum distances from set P
        Xp = [P(1:sP(1)) Q(ixp)];
    ...
    elseif sP(2) == 3
        h(1) = plot3(P(:,1),P(:,2),P(:,3),'bx','markersize',3,'linewidth',3);
        h(2) = plot3(Q(:,1),Q(:,2),Q(:,3),'ro','markersize',3,'linewidth',2.5);
        % draw all minimum distances from set P
        Xp = [P(1:sP(1),1) Q(ixp,1)];
        Yp = [P(1:sP(1),2) Q(ixp,2)];
