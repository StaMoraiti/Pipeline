...
% b=[0,0,1];
% c=[0,1,0];
function [NDcr,NDmu,NDstd]=NormalDist(surf1,surf2)
aa=surf1.vertices;
aafaces=surf1.faces;
...
MaxDba=max(Dba);
MuDba=mean(Dba); STDDba=std(Dba);
NDcr=max(MaxDab,MaxDba);
NDmu=max(MuDab,MuDba);
NDstd=max(STDDab,STDDba);
