...
function  E_av_sd_AllNodes_Limited(mouse, week, dl, NS)
%% Error
...
for m=1:size(ind_b,1)
    d_slice=d_3D(:,:,ind_b(m,3))';
    d1_s1=[d1_s1;d_slice(ind_b(m,1),ind_b(m,2))];
end

% Imposed Displacement
% cd(strcat(pathres,pathSpec,pathNS));cd("..\")
% load('ImposedMode.mat','d_hat');
...
cd(strcat(pathres,pathSpec,pathNS));
save(filename, 'ErrorS_avL','ErrorS_stdL','ErrorS')
end
