...
def make_roi(event):
    global roi, ofl, canvas, roinum, dispath
    ofile = open(dispath + ofl + str(roinum) + '.txt',"w")
    ln = len(roi)
    roi = roi + [roi[0]]
    ...
    ln = len(roi)
    while count < ln:
        ofile.write("{" + str(roi[count][0]+1) + " " + str(roi[count][1]+1) + "} ")
        count = count + 1
    ofile.close()
    ofile = open(dispath + ofl + 'num.txt',"w")
...
cols, rows, m = load_bmp(dispath+fl+str(middle))
canvas_width = cols
canvas_height = rows
roi = []
